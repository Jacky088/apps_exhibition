...
### v2.0.9
- 安全加固：Swiper 改为本地加载并锁定 10.3.1（规避 CDN 供应链风险与国内访问不稳定导致的轮播失效）；后台 JS 输出转义修复；下载链接与海报 URL 增加 http/https 协议白名单
- 修复表单保存缺少 wp_unslash 导致含引号输入（如 Tom's App）入库残留反斜杠、重复编辑不断累积的问题
- 升级时自动迁移清洗历史数据中的反斜杠（DB_VERSION 1.0.1，升级后首次访问自动执行）
- 保存失败时在列表页保留并展示具体校验错误，不再只显示笼统的失败提示
- 插件版本提升清理缓存时同步删除 `_transient_timeout_` 孤儿行，避免 options 表缓慢膨胀
- 修复移动端长文字分类滚动到边缘时激活胶囊蓝色背景右端被白色渐变遮罩漂白（显示不完整）的问题：改用 CSS mask 右缘自然渐隐，并移除 iOS 冗余的 `-webkit-overflow-scrolling`（其合成滚动层存在吞末端 padding 的怪癖）
- 后台新增「批量移动」功能：批量勾选应用后可将指定源分类替换为新分类（适合分类重命名后的数据迁移），也可选择"全部"整体覆盖；目标分类下拉与「分类设置」的筛选分类实时同步；排序分类下拉补充显示"使用中但未在设置里"的旧分类名，便于筛选待迁移应用
