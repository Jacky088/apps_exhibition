...
- 搜索过滤 + 批量删除
- 删除应用 / 移除海报时自动清理媒体库中的孤儿图片（仍被引用的图片不会误删）
- 多端自适应（桌面/平板/手机）
...
### v2.0.11
- 新增媒体库孤儿图片自动清理：删除应用（单个/批量）时自动删除媒体库中不再被任何应用图标或首页海报引用的 logo 图片，移除海报时同样清理对应海报图片
- 清理前做双重引用检查（应用图标 + 首页海报），共用图片不会被误删；外链图片自动跳过；删除时连同缩略图尺寸一并移除
